        ...
        if (spawnPos.Local.IsOrigin())
        {
            ...
            if (slaveTemplate.IsABoat())
            {
                ...
                for (var inFront = 0f; inFront < 50f + tempShipModel.MassBoxSizeX; inFront += 1f)
                {
                    ...
                    var floorHeight = World.GetHeight(depthCheckPos.World.Position);// WorldManager.Instance.GetHeight(depthCheckPos);
                    if (floorHeight > 0f)
                    {
            ...
            else
            {
                ...
                var h = World.GetHeight(spawnPos.World.Position);// WorldManager.Instance.GetHeight(spawnPos);
                if (h > 0f)
