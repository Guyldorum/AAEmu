...
public class ExperienceManagerTests
{
    ...
    public async Task GetLevelFromExp_Valid(int exp, bool mate, int expectedLevel, int expectedOverflow)
    {
        var level = _cut.GetLevelFromExp(exp, out var overflow, mate);
        await Assert.That(level).IsEqualTo(expectedLevel);
        await Assert.That(overflow).IsEqualTo(expectedOverflow);
    ...
    public async Task GetLevelFromExp_CurrentLevel_Valid(int exp, bool mate, byte currentLevel, int expectedLevel)
    {
    ...
    public async Task Load_ResetsState()
    {
        ...
        // assert
        await Assert.That(level).IsEqualTo(1); // should be level 1 if the second loader overwrote the first
        await Assert.That(_cut.MaxPlayerLevel).IsEqualTo(1);
        await Assert.That(_cut.MaxMateLevel).IsEqualTo(1);
        mockLoader1.Verify();
        mockLoader2.Verify();
    }
